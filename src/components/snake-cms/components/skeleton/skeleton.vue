<template>
  <view class="skeleton" :style="[customStyle]">
    <view class="skeleton__wrapper" ref="skeleton__wrapper" v-if="loading" :style="{ display: 'flex', flexDirection: direction }">
      <view
        class="skeleton__wrapper__avatar"
        v-if="avatar"
        :class="[`skeleton__wrapper__avatar--${avatarShape}`, animate && 'animate']"
        :style="{
          height: avatarSize + 'px',
          width: avatarSize + 'px',
          marginBottom: '10px',
        }"></view>
      <view class="skeleton__wrapper__content" ref="skeleton__wrapper__content" style="flex: 1">
        <view
 ...
 * @example <skeleton rows="3"></skeleton>
 */
export default {
  name: 'skeleton',
  props: {
  ...
  computed: {
    rowsArray() {
      ...
      for (let i = 0; i < this.rows; i++) {
        const item = {};
        // 需要预防超出数组边界的情况
        const rowWidth = this.$u.test.array(this.rowsWidth)
          ? this.rowsWidth[i] || (i === this.row - 1 ? '70%' : '100%')
          : i === this.rows - 1
            ? '70%'
            : this.rowsWidth;
        const rowHeight = this.$u.test.array(this.rowsHeight) ? this.rowsHeight[i] || '18px' : this.rowsHeight;
        ...
        if (/%$/.test(rowWidth)) {
          ...
          item.width = (this.width * parseInt(rowWidth)) / 100 + 'px';
        } else {
          item.width = rowWidth + 'px';
        }
    ...
    uTitleWidth() {
      ...
      if (/%$/.test(this.titleWidth)) {
        // 通过parseInt提取出百分比单位中的数值部分，除以100得到百分比的小数值
        tWidth = (this.width * parseInt(this.titleWidth)) / 100;
      } else {
        tWidth = this.titleWidt;
      }
      return tWidth + 'px';
    },
  },
  ...
  methods: {
    ...
    async setNvueAnimation() {
      ...
      const skeleton = this.$refs.skeleton__wrapper;
      skeleton &&
        this.loading &&
        this.animate &&
    ...
    async getComponentWidth() {
      ...
      // #ifndef APP-NVUE
      this.$uGetRect('.skeleton__wrapper__content').then((size) => {
        this.width = size.width;
      });
      // #endif

      // #ifdef APP-NVUE
      const ref = this.$refs.skeleton__wrapper__content;
      ref &&
...
@mixin background {
  ...
  /* #endif */
  /* #ifndef APP-NVUE */
  background: linear-gradient(90deg, rgba(219, 220, 226, 0.5) 25%, rgba(219, 220, 226, 0.8) 37%, rgba(219, 220, 226, 0.6) 50%);
